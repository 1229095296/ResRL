...
class RayPPOTrainer:
    ...
    def _save_best_checkpoint(self, aime_score=None, livecodebench_score=None):
        ...
        if aime_score is not None and aime_score > self.best_aime_score:
            ...
            best_ckpt_dir = os.path.join(self.config.trainer.default_local_dir, "best_aime_ckpt")
            local_mkdir_safe(best_ckpt_dir)

            # Save actor checkpoint
            actor_local_path = os.path.join(best_ckpt_dir, "actor")
            actor_remote_path = (
                None
                if self.config.trainer.default_hdfs_dir is None
                else os.path.join(self.config.trainer.default_hdfs_dir, "best_aime_ckpt", "actor")
            )

            print(f"Saving best AIME checkpoint with score {aime_score:.4f} at step {self.global_steps}")
            self.actor_rollout_wg.save_checkpoint(
                actor_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=1
            )

            # Save critic checkpoint if applicable
            if self.use_critic:
                critic_local_path = os.path.join(best_ckpt_dir, "critic")
                critic_remote_path = (
                    None
                    if self.config.trainer.default_hdfs_dir is None
                    else os.path.join(self.config.trainer.default_hdfs_dir, "best_aime_ckpt", "critic")
                )
            ...
            metadata_path = os.path.join(best_ckpt_dir, "best_checkpoint_info.txt")
            with open(metadata_path, "w") as f:
                f.write(f"Best AIME Combined Score: {aime_score:.4f}\n")
                f.write(f"Global Step: {self.global_steps}\n")

            self.best_aime_ckpt_path = best_ckpt_dir
        ...
        if livecodebench_score is not None and livecodebench_score > self.best_livecodebench_score:
            ...
            best_ckpt_dir = os.path.join(self.config.trainer.default_local_dir, "best_livecodebench_ckpt")
            local_mkdir_safe(best_ckpt_dir)
            ...
            actor_remote_path = (
                None
                if self.config.trainer.default_hdfs_dir is None
                else os.path.join(self.config.trainer.default_hdfs_dir, "best_livecodebench_ckpt", "actor")
            )

            print(f"Saving best livecodebench checkpoint with score {livecodebench_score:.4f} at step {self.global_steps}")
            self.actor_rollout_wg.save_checkpoint(
                actor_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=1
            )
            ...
            if self.use_critic:
                ...
                critic_remote_path = (
                    None
                    if self.config.trainer.default_hdfs_dir is None
                    else os.path.join(self.config.trainer.default_hdfs_dir, "best_livecodebench_ckpt", "critic")
                )
                self.critic_wg.save_checkpoint(
                    critic_local_path, critic_remote_path, self.global_steps, max_ckpt_to_keep=1
                )
            ...
            metadata_path = os.path.join(best_ckpt_dir, "best_checkpoint_info.txt")
            with open(metadata_path, "w") as f:
                f.write(f"Best livecodebench/reward/mean@1 Score: {livecodebench_score:.4f}\n")
                f.write(f"Global Step: {self.global_steps}\n")

            self.best_livecodebench_ckpt_path = best_ckpt_dir
    ...
    def _save_checkpoint(self):
        ...
        # For periodic global_step checkpoints, we always keep all of them (max_ckpt_to_keep=None)
        # Best checkpoints are managed separately in _save_best_checkpoint() with max_ckpt_to_keep=1
