...
def safe_torch_save(obj, path, max_retries=3, retry_delay=2.0):
    """
    Safely save a PyTorch object to a file with retry mechanism.
    ...
    # Add a small delay to ensure directory is visible across nodes
    time.sleep(0.1)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if os.path.exists(path):
                ...
                raise RuntimeError(f"File {path} was not created after torch.save()")

        except Exception as e:
            if attempt < max_retries - 1:
                logger.warning(f"Failed to save {path} (attempt {attempt + 1}/{max_retries}): {e}. Retrying...")
                time.sleep(retry_delay)
            else:
                logger.error(f"Failed to save {path} after {max_retries} attempts: {e}")
                raise
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        # Get the parent directory of local_path to manage checkpoints per directory
        parent_path = os.path.dirname(local_path)

        # Initialize path tracking per parent directory if not exists
        if not hasattr(self, '_previous_saved_paths_by_dir'):
            self._previous_saved_paths_by_dir = {}

        if parent_path not in self._previous_saved_paths_by_dir:
            self._previous_saved_paths_by_dir[parent_path] = []

        # remove previous local_path, only rank 0 should do this
        # But only for checkpoints in the same parent directory
        if (
            self.rank == 0
            and max_ckpt_to_keep
            and isinstance(max_ckpt_to_keep, int)
            and max_ckpt_to_keep > 0
            and len(self._previous_saved_paths_by_dir[parent_path]) >= max_ckpt_to_keep
        ):
            keep_start = len(self._previous_saved_paths_by_dir[parent_path]) - max_ckpt_to_keep + 1
            self.remove_previous_save_local_path(self._previous_saved_paths_by_dir[parent_path][:keep_start])
            self._previous_saved_paths_by_dir[parent_path] = self._previous_saved_paths_by_dir[parent_path][keep_start:]
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    safe_torch_save(model_state_dict, model_path)
                    log_with_rank(f"Saved model to {os.path.abspath(model_path)}", rank=self.rank, logger=logger)

                if self.should_save_optimizer:
                    optimizer_state_dict = self.optimizer.state_dict()
                    safe_torch_save(optimizer_state_dict, optim_path)
                ...
                if self.should_save_extra:
                    ...
                    safe_torch_save(extra_state_dict, extra_path)
        ...
        # Append to the directory-specific list
        self._previous_saved_paths_by_dir[parent_path].append(local_path)
        # Also maintain the old list for backward compatibility
